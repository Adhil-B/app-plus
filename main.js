...
if (window.location.pathname.includes('/Food/Index.aspx')) {
    const fixedCSS = `
        ...
        body, .ui-page, .ui-header, .ui-footer, .ui-content {
    ...
    const iconMapping = {
        'Food Courts': 'restaurant',
        'Offers': 'local_offer',
        'Restaurants': 'restaurant',  // Changed from storefront
        'Order Status': 'pending_actions',
        'Add Cash': 'account_balance_wallet',
        'Feedback': 'feedback',
        'Reports': 'analytics',
    ...
    // Update icons with proper implementation
    document.querySelectorAll('[data-role="button"]').forEach(button => {
    ...
    document.querySelectorAll('[data-role="navbar"] a').forEach(link => {
        ...
        link.innerHTML = `
            <span class="material-icons" style="font-size:1.2em">${icon}</span>
